...
const { richmdCli } = require("richmd");
const fs = require("fs");
...
const richmdText = (path, dir, name) => {
  ...
  fs.readFile(path, { encoding: "utf8" }, (err, file) => {
    if (file !== undefined) {
      text += richmdCli(file);
      text += `</body>\n</html>`;
      fs.writeFile(`${program.opts().dir}/${name[0]}.html`, text, (err, data) => {
...
program
  .version("1.0.4")
  .usage("[options] <file ...>")
  .option("-d, --dir [path]", "Add OutPut Directory", "./")
  .parse(process.argv);

const dirHierarchy = program.opts().dir.trim().split(`\/`);
let relativePath = "";
...
<link rel="stylesheet" href="${relativePath}node_modules/richmd/richmd.css">
`;
...
if (!lastFilePath) {
  dirList(filePath, program.opts().dir);
} else {
  if (!/\./.test(lastFilePath)) {
    ...
    if (filename[1] !== "richmd") {
      ...
      richmdText(filePath, program.opts().dir, filename);
    }
  }
}
